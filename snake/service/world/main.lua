if is_ks_server() then
    local ks_main = import(service_path("kuafu.ks_main"))
    return
end
...
local feedback = import(service_path("feedbackmgr"))
...
local Gzmgr = import(service_path("Gzmgr")) --定义接口文件入口变量
...
skynet.start(function()
    ...
    global.oGzMgr = Gzmgr.NewGzMgr()  --定义接入文件入口
    ...
    gamedb.LoadDb("world", "common", "DbOperate", mInfo, function (mRecord, mData)
            global.oWorldMgr = worldobj.NewWorldMgr()
            global.oWorldMgr:Load(mData.data)
            global.oWorldMgr:OnLoaded()
            ...
            local mLoading = {}
            local wait_load = function (sName, o)
                mLoading[sName] = true
                ...
                if sName == "org" or sName == "huodong" then
                    o:Execute(loaded_func)
                else
                    o:WaitLoaded(loaded_func)
                end
            end
            ...
            global.oKuaFuMgr = kuafumgr.NewKuaFuMgr()
            global.oKuaFuMgr:LoadDB()
            wait_load("kuafu", global.oKuaFuMgr)
            ...
            global.oFeedBackMgr = feedback.NewFeedBackMgr()
