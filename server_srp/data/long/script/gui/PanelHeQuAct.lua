...
local actType = 2  --actType:1(庆典活动)  actType:2(合区活动) --根据actType类型显示页签
...
local tabTable2 = {
	...
	{index=8,  name="狼牙回馈", nameStr="tabName8",  openDays=5, func=function(player) return (server_merge_day()<=4 and server_merge_day()>=0 and actType==2) end},
	{index=9,  name="龙心回馈", nameStr="tabName9",  openDays=5, func=function(player) return (server_merge_day()<=4 and server_merge_day()>=0 and actType==2) end},
	{index=10, name="沙城争霸", nameStr="tabName10", openDays=5, func=function(player) return (server_merge_day()<=4 and server_merge_day()>=0 and actType==2) end},
	{index=11, name="神秘商店", nameStr="tabName11", openDays=3, func=function(player) return (server_merge_day()<=4 and server_merge_day()>=0 and actType==2) end},
}
...
function onPanelData(player, commond)
	...
	elseif data.actionid=="reqRuiShowGo" then--瑞兽传送
		if mergeTabOpenState("超级瑞兽") then
			player:enter_map("biqi", 32, 86)
			player:push_lua_table("close",util.encode({name = "extend_heFu"}))
		else
...
ckpanel.add_listener("gui.PanelHeQuAct.onPanelData",gui.PanelHeQuAct.onPanelData)
...
local rechargeAwards = {
	[1]={
		...
		{id = 23020002,	num = 1, bind = 1},
		{id = 23140003,	num = 2, bind = 1},
		{id = 32000002,	num = 1, bind = 1},
		{id = 32000003,	num = 1, bind = 1},
	},
	[2]={
		{id = 23040001,	num = 1, bind = 1},
		{id = 23030002,	num = 1, bind = 1},
		{id = 23140003,	num = 2, bind = 1},
		{id = 32000002,	num = 1, bind = 1},
		{id = 32000003,	num = 1, bind = 1},
	},
...
function updateRechargeData(player)
	local result = {
		...
		ling=0,
		time=0,
		awards=util.clone(rechargeAwards[3])
	}
	if mergeTabOpenState("充值有礼") then
		if rechargeAwards[server_merge_day()+1] then
			result.awards=util.clone(rechargeAwards[server_merge_day()+1])
		end
	end
	if util.ppn(player,const.PP_HEQU_DAILY_CZ)>=1000 then
		result.ling=1--可领取
	end
	if util.ppn(player,const.PP_DAILY_RECHARGE_LING)>0 then
		result.ling=2--已领取
	end
	if server_merge_day()<=2 and server_merge_day()>=0 then
		local H = tonumber(os.date("%H",os.time()))
		local M = tonumber(os.date("%M",os.time()))
		local S = tonumber(os.date("%S",os.time()))
...
function getRechargeData(player)
	local tempTable = util.clone(rechargeAwards[3])
	if mergeTabOpenState("充值有礼") then
		if rechargeAwards[server_merge_day()+1] then
			tempTable=util.clone(rechargeAwards[server_merge_day()+1])
		end
	else
		return player:alert(1,1,"活动已结束！")
	end
	if util.ppn(player,const.PP_HEQU_DAILY_CZ)<1000 then
		player:alert(1,1,"今日充值不足1000元宝！")
		return
	end
	if util.ppn(player,const.PP_DAILY_RECHARGE_LING)>0 then
...
function setExpBuff(player)
	if gui.PanelCelebrationAct.checkActIng() or mergeTabOpenState("全服双倍") then
		local H = tonumber(os.date("%H",os.time()))
		local M = tonumber(os.date("%M",os.time()))
		local S = tonumber(os.date("%S",os.time()))
		local yuTime1 = (gui.PanelCelebrationAct.formattime(get_day_YMD(),gui.PanelCelebrationAct.endYMD)-1)*24*3600+(24-H)*3600-M*60-S
		local yuTime2 = 0
		if server_merge_day()>=0 then
			yuTime2 = (3-server_merge_day()-1)*24*3600+(24-H)*3600-M*60-S
		end
		local yuTime=0
		if yuTime1>yuTime2 then
			yuTime=yuTime1
		else
			yuTime=yuTime2
		end
		if yuTime>=0 then
			player:clear_buff(25101)
			player:set_buff(25101, (os.time()+ yuTime))
		end
	end
end
login.add_login_listener(setExpBuff)
...
function updateLotteryData(player)
	...
	-- if checkActIng()then
		...
		result.time=(formattime(get_day_YMD(),endYMD)-1)*24*3600+(24-H)*3600-M*60-S
		if actType==2 then
			result.time=(4-server_merge_day())*24*3600+(24-H)*3600-M*60-S
		end
	-- end
	player:push_lua_table("PanelHeQuAct",util.encode(result))
end
...
local zheKouTable = {
	...
	[3]={
		...
		{id=23030001, name="狼牙碎片（小）", moneyType=102, pricey=1000, price=799, num=1,   bind=1, allnum=3, svr="svr_curbuynum2", con=const.PP_QIANGGOU_BUYNUM2},
		{id=24030001, name="羽毛",           moneyType=102, pricey=6000, price=4800, num=200, bind=1, allnum=3, svr="svr_curbuynum3", con=const.PP_QIANGGOU_BUYNUM3},
	},
	[4]={
...
local zheKouTable2 = {
	[1]={
		{id=24030001, name="羽毛",           moneyType=102, pricey=11400, price=8999, num=380, bind=1, allnum=3, svr="svr_curbuynum1", con=const.PP_QIANGGOU_BUYNUM1},
		{id=23020002, name="龙心碎片（中）", moneyType=102, pricey=11000, price=8888, num=11,  bind=1, allnum=3, svr="svr_curbuynum2", con=const.PP_QIANGGOU_BUYNUM2},
		{id=23030002, name="狼牙碎片（中）", moneyType=102, pricey=13000, price=9999, num=13,  bind=1, allnum=3, svr="svr_curbuynum3", con=const.PP_QIANGGOU_BUYNUM3},
	}
}
...
function buyZheKouGoods(player,index)
	if not mergeTabOpenState("限时抢购") then return player:alert(1,1,"活动已结束！") end
...
function updateLangYaData(player)
	...
	result.time=(5-mergeDay)*24*3600+(24-H)*3600-M*60-S
...
--------------------------------------------------------------沙城争霸------------------------------------------------------------------
local cityTable={
	[1]={svr="svr_sczb_ling1", mergeDay=3, awards={{id=27020004, num=1, bind=1},{id=40000005, num=10000, bind=0}} },
	[2]={svr="svr_sczb_ling2", mergeDay=5, awards={{id=27020005, num=1, bind=1},{id=40000005, num=10000, bind=0}} },
...
function getWangChengAwards(player,index)
	...
	if mergeDay~=itemData.mergeDay then
		player:alert(1,1,"合服第"..itemData.mergeDay.."天沙城争霸结束方可领取！")
		return
	end
...
--邮件发放沙城争霸奖励
function sendMailSczb(player)
	local mergeDay = server_merge_day()+1
	if mergeDay==3 or mergeDay==5 then
		local myName = player:get_name()
		local candidate = server.get_king_guild()
		local leaderOfKing = ""
		if candidate and candidate ~= ""  then
		...
		if myName==leaderOfKing then
			if mergeDay==3 then
				util.sendMail(player,"沙城争霸", "您在合服第3天获得沙城争霸城主奖励", cityTable[1].awards)
			elseif mergeDay==5 then
				util.sendMail(player,"沙城争霸", "您在合服第5天获得沙城争霸城主奖励", cityTable[2].awards)
			end
		end
	end
end
...
function checkRedPointLangYa(player)
	local result = false
	local allUse=util.ppn(player,const.PP_LANGYA_USEB_NUM)--已使用次数
	local dataTable=util.clone(lyTable)
	for i=1,#dataTable do
		local itemData = dataTable[i]
		itemData.ling = util.ppn(player,itemData.con)
		if allUse>=itemData.useNum and itemData.ling<=0 then
			result = true
			break
		end
	end
	player:push_lua_table("PanelHeQuAct",util.encode({cmd="updateTabRed",name="tabName8",show=result}))
	return result
end
...
function checkRedPointLongXin(player)
	local result = false
	local allUse=util.ppn(player,const.PP_LONGXIN_USEB_NUM)
	...
	player:push_lua_table("PanelHeQuAct",util.encode({cmd="updateTabRed",name="tabName9",show=result}))
	return result
end
...
function checkRedPointRecharge(player)
	local result = false
	if util.ppn(player,const.PP_HEQU_DAILY_CZ)>=1000 and util.ppn(player,const.PP_DAILY_RECHARGE_LING)<=0 then
		result = true
	end
	player:push_lua_table("PanelHeQuAct",util.encode({cmd="updateTabRed",name="tabName2",show=result}))
	return result
end
