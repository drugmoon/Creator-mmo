...
function CTask:ValidFight(iPid,npcobj)
    ...
    if oPlayer:GetMemberSize() + oPlayer:Query("testman", 0) < 1 then
...
function CTask:TransReward(oAwardee,sReward, mArgs)
    ...
    mArgs.iBaseExp = math.floor(iValue * (500 + iSize) / taskdefines.GHOST_INFO.ROUND_RINGS * iBase)
    return  iValue * (100 + iDouble + iLeader ) * (500 + iSize) / taskdefines.GHOST_INFO.ROUND_RINGS * math.floor(iBase)
