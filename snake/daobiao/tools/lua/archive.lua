...
local function dumpfile(filename, packname)
    ...
    local oldcode = old_data[packname]
    if oldcode and oldcode==codedump then
      print("keep:", packname)
      -- local size = #packname + 1
      -- return lenstr(size)..packname.."\0"
      return nil
    else
      local size = lfs.attributes(filename, "size")
      totalsize = totalsize + size
      print(size, "------------new:", packname)
